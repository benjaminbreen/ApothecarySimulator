  ...
  const allLayerInstances = useMemo(() => {
    ...
    return cloudConfig.layers.map((layer, layerIndex) => {
      ...
      for (let i = 0; i < layer.count; i++) {
        ...
        const scale = sizeMultiplier * (1.8 + rng.random() * 0.4);
                    ...
                    return (
                      ...
                      >
                        <defs>
                          ...
                          <radialGradient id={highlightId} cx={`${lighting.highlightCx}%`} cy={`${lighting.highlightCy}%`} r="70%">
                            <stop offset="0%" stopColor={colors.highlight} stopOpacity="0.7" />
                            <stop offset="60%" stopColor={colors.main} stopOpacity="0.25" />
                        ...
                        {isPuffCloud && (
                          <g mask={`url(#${maskId})`}>
                            ...
                            {/* Main cloud body - IMPROVED: Density gradients for realistic falloff */}
                            <g>
                              {instance.shapeData.puffs.map((puff, idx) => (
                                <ellipse
                                  key={`main-${idx}`}
                            ...
                            {/* Wispy outer edges - IMPROVED: Variable softness for realistic cloud edges */}
