// SimpleInteractionTestPanel.jsx
// UI component for running simple interaction continuation tests

import React, { useState, useRef, useEffect } from 'react';
import { runAllTests, TEST_SCENARIOS } from '../tests/simpleInteractionTests';

const SimpleInteractionTestPanel = ({ gameState }) => {
    ...
    <div className="test-runner p-4 bg-gray-50 dark:bg-slate-800 rounded-lg">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Automated tests for simple interaction continuation narratives (water sellers, beggars, information exchanges).
          <br />
          <strong>Note:</strong> These tests call the actual LLM and may take 1-2 minutes.
        </p>
        ...
        {currentTest && (
          <div className="mt-3 p-2 bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 rounded text-sm">
              ...
              <li key={scenario.id} className="flex items-start">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {scenario.description} (Choice: {scenario.simpleInteractionChoice}, {scenario.checks.length} checks)
                  </div>
                </div>
              </li>
            ))}
        ...
        <div className="mb-4 p-4 bg-white dark:bg-slate-700 rounded-lg border-2 border-gray-200 dark:border-slate-600">
          ...
          <div className="grid grid-cols-4 gap-3 mb-4">
            ...
            <div className="text-center p-3 bg-purple-100 dark:bg-purple-900 rounded">
              ...
              <div className="text-xs text-purple-600 dark:text-purple-400">Scenarios</div>
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-gray-600 dark:text-gray-400">
                        ...
                        {scenario.interactionResult && ` • ${scenario.interactionResult.type}`}
                      </div>
                    </div>
                  </div>
                  <svg
                    ...
                    {scenario.interactionResult && (
                      <div className="mb-4 p-3 bg-purple-50 dark:bg-purple-900/20 rounded border border-purple-200 dark:border-purple-800">
                        ...
                        <div className="text-xs text-purple-900 dark:text-purple-200 space-y-1">
                          <div><strong>Type:</strong> {scenario.interactionResult.type}</div>
                          <div><strong>Choice:</strong> {scenario.interactionResult.choice}</div>
                          <div><strong>NPC:</strong> {scenario.interactionResult.npcName}</div>
                        </div>
                      </div>
                    )}
                          ...
                          <div key={idx} className="text-xs bg-white dark:bg-slate-800 p-2 rounded">
                            ...
                            {turn.hasSimpleInteraction && (
                              <div className="text-green-700 dark:text-green-300 mt-1">
                                ✓ Simple interaction triggered: {turn.interactionType}
                              </div>
                            )}
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {scenario.warnings.map((warning, idx) => (
                            <div
                              key={idx}
                              className="text-xs p-2 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 rounded"
                            >
                              {warning}
                            </div>
                          ))}
      ...
      {logs.length > 0 && (
        <div className="mt-4">
          ...
          <div
            ref={logContainerRef}
            className="bg-black text-purple-400 p-3 rounded font-mono text-xs h-64 overflow-y-auto"
          >
            {logs.map((log, idx) => (
...
export default SimpleInteractionTestPanel;
